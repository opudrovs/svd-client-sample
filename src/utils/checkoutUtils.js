...
export const getBundleLicense = (bundleId, licenseId) => {
  const products = BUNDLE_PRODUCTS.filter(
    (elementProduct) => elementProduct.elementId === bundleId
  );

  return products.length > 0
    ? products[0].productLicenses.filter(
        (elementLicense) => elementLicense.id === licenseId
      )[0]
    : null;
};

export const getPackLicense = (licenseId) => {
  return PACK_PRODUCTS.filter(
    (elementLicense) => elementLicense.id === licenseId
  )[0];
};
